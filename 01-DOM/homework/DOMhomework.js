...
creator.textContent = 'Aplicación creada por: Sonny';
...
function buildToDo(todo, index) {
  ...
  check.type = 'checkbox';
  check.id = index;
  check.classList = 'completeCheckbox';
...
function buildToDos(toDos) {
  // Tu código acá:
  return toDos.map( (current,index) => buildToDo(current,index) );
}
...
function addToDo() {
  ...
  if(!input.value) return alert('Ingresar una tarea')
